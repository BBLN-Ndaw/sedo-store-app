...
class ProductService(
     ...
     */
    fun clearProductImages(productId: String): Product {
        val currentUser = SecurityContextHolder.getContext().authentication.name
        logger.info("Clearing all images from product ID: {} by user: {}", productId, currentUser)

        val existingProduct = getProductById(productId)
        val oldImageCount = existingProduct.images.size

        val updatedProduct = existingProduct.copy(images = emptyList())
        val savedProduct = productRepository.save(updatedProduct)
        ...
        auditService.logAction(
            ...
            entityType = "Product",
            entityId = savedProduct.id,
            description = "All images cleared from ${savedProduct.name}",
            oldData = mapOf("imageCount" to oldImageCount.toString()),
            newData = mapOf("imageCount" to "0")
        )

        logger.info("All images cleared successfully from product: {} (ID: {})", savedProduct.name, savedProduct.id)
        return savedProduct
    }

    /**
     * Remplace toutes les images d'un produit
     */
    fun replaceProductImages(productId: String, newImageUrls: List<String>): Product {
        val currentUser = SecurityContextHolder.getContext().authentication.name
        logger.info("Replacing all images for product ID: {} with {} new images by user: {}", productId, newImageUrls.size, currentUser)

        val existingProduct = getProductById(productId)
        val oldImageCount = existingProduct.images.size

        val updatedProduct = existingProduct.copy(images = newImageUrls)
        val savedProduct = productRepository.save(updatedProduct)

        auditService.logAction(
            userName = currentUser,
            action = "IMAGES_REPLACE",
            entityType = "Product",
            entityId = savedProduct.id,
            description = "All images replaced for ${savedProduct.name}",
            oldData = mapOf("imageCount" to oldImageCount.toString()),
            newData = mapOf("imageCount" to newImageUrls.size.toString())
        )

        logger.info("Images replaced successfully for product: {} (ID: {})", savedProduct.name, savedProduct.id)
        return savedProduct
    }
}
